...
Dic{i}='270,AnimalID'; i=i+1; % originally imageDescription
...
for j=1:length(Dic)
    bytes=uint8(Dic{j});
    bytes(bytes==32)=[]; % delete space
    n=find(bytes==44); % find comma
...
for j=1:IfdLength
    ...
    % If more bytes than 4 OR data type is ASCII, the data is stored
    % elsewhere in the file - Modified by Yueqi (2016)
    if( nbyte>4 || ismember(TagCode,uint16([270,271,305,306,315,33432,65000])) )
